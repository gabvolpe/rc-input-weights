"""
Sine-to-Cosine^2 — Unconditional Variability Extraction, fixed Reservoir | Variable Read-in.
...
def save_checkpoint(output_dir, readin_records, pred_records, reservoir_records, y_test):
    ...
    Files written
    -------------
    sc2_ground_truth.npy          shape (n_time, n_states)  — saved every call but never changes
    sc2_reservoir_weights.npy     shape (n_outer_so_far, 1 + n_reservoir_weights)
    sc2_readin_weights_{dist}.npy shape (n_rows, 2 + n_nodes)   columns: outer, inner, w...
    sc2_timeseries_{dist}.npy     shape (n_rows, 2 + n_time)    columns: outer, inner, t...
    sc2_timeseries_gt.npy         shape (n_rows, 2 + n_time)    columns: outer, inner, t...
    """
    # Ground truth — constant, written once (overwrite is a no-op after first call)
    np.save(os.path.join(output_dir, "sc2_ground_truth.npy"), y_test[0])
    ...
    _save_records(readin_records, os.path.join(output_dir, "sc2_readin_weights_{}.npy"))
    _save_records(pred_records,   os.path.join(output_dir, "sc2_timeseries_{}.npy"))

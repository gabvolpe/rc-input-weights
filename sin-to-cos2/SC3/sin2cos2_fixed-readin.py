...
1)The readin weights are saved in the corresponding .npy, in the form: [readin_weights]
...
parser.add_argument("--set_threshold", type=bool, default=False, help="Set to False if no threshold wished")
...
def main():
    ...
    for trial_outer in range(args.n_trials):
        ...
        W_uniform = create_weights((args.reservoir_nodes, 1), "random_uniform")
        W_gauss = create_weights((args.reservoir_nodes, 1), "random_normal", dynamic_sd=1.0)
        W_dbgauss = create_weights((args.reservoir_nodes, 1), "double_gaussian", dynamic_sd=1.0)
        W_laplace = create_weights((args.reservoir_nodes, 1), "laplace")
        W_powlaw = create_weights((args.reservoir_nodes, 1), "power_law")

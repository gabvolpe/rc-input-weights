...
def run_inner_trial(model_serialized, sd_list, X_train, y_train, X_test, y_test):
    ...
    # Power-law
    weights_power = create_weights((200,1), "power_law")
...
def main():
    ...
    sheet_name_median = 'Exp'+args.constraint_set+'_Sine Wave_Median'
    sheet_name_iqr = 'Exp'+args.constraint_set+'_Sine Wave_IQR'
    ...
    #sd_list = eval(args.sd_list)# [0.1, 0.25, 0.5, 0.75, 1.0]

    # Initialize losses dictionary correctly for all trials
    losses = {key: [[] for _ in range(args.n_trials)] for key in ["Uniform"] + [f"Gaussian_sd_{sd}" for sd in sd_list] + ["Power-Law","Double-Gaussian","Laplace"]}
    ...
    for trial_outer in range(args.n_trials):
        ...
        double_gaussian_inner_predictions = []
        for _ in range(args.n_inner):
            double_weights = create_weights((200,1), "double_gaussian", dynamic_sd=best_sd_numeric)
            ...
            double_gaussian_inner_predictions.append(y_pred)
        losses["Double-Gaussian"][trial_outer] = double_gaussian_inner_losses
        predictions["Double-Gaussian"][trial_outer] = double_gaussian_inner_predictions

        
        # ------------PLOTTING------------------------------
        
        # 1a. Reservoir network plot for selected outer trial        
        from matplotlib.lines import Line2D
        print("\nTarget vs Prediction plot..")
        washout = 200  # initial timesteps (transients) to discard
        plt.figure(figsize=(12, 6))

        # Target signal
        plt.plot(
            range(washout, y_test.shape[1]),
            y_test[0, washout:, 0],
            label="Target Signal",
            linewidth=2,
        )
        ...
        method_list = (
            ...
            + ["Double-Gaussian", "Laplace", "Power-Law"]
        )
        colors = ["orange", "green", "red", "purple", "brown", "yellow", "cyan", "navy", "pink"]
        ...
        for color, method in zip(colors, method_list):
            ...
            if method == "Double-Gaussian":
                # average across inner trials
                y_pred_plot = np.mean(preds_for_outer, axis=0)
            else:
                # take the first inner trial (index 0)
                y_pred_plot = preds_for_outer[0]

            # Determine prediction length and values
            ...
            for k, key in enumerate(predictions.keys()):
                if len(predictions[key][trial_outer]) > 0:
                    y_pred = predictions[key][trial_outer][0]
                    plt.plot(range(washout, y_pred.shape[1]), y_pred[0, washout:, 0], label=key, color=colors[k % len(colors)], linewidth=1)
    ...
    for trial_outer in range(args.n_trials):
        ...
        for method, col in model_columns.items():
            ...
            if not all_losses:
                continue
            # Compute median and IQR across inner trials
            median, iqr = compute_median_and_iqr(all_losses)
    ...
    # --- Write best Gaussian SD as comment in first cell of column 10 ---

...
for dist in DISTRIBUTIONS:
    ...
    preds[dist]=np.load(
        os.path.join(
            RESULTS_DIR,
            f"sc{SET_CONSTRAINT}_timeseries_{dist}.npy"
        ),
        allow_pickle=True
    )
...
outer_ids=sorted(
    set(
        preds[DISTRIBUTIONS[0]][:,0].astype(int)
    )
)
...
ax.set_xticklabels(
    [f"Reservoir {i}" for i in outer_ids]
)
...
for dist in DISTRIBUTIONS:
    ...
    best_preds[dist]={
        "best":
            (
                ts_all[np.argmax(scores)],
                scores.max()
            ),
        "median":
            np.median(ts_all,axis=0),
        "lo":
            ts_all.min(axis=0),
        "hi":
            ts_all.max(axis=0)
...
for dist in DISTRIBUTIONS:
    ...
    ax.plot(
        ...
        ts,
        color=c,
        label=f"{DIST_LABELS[dist]} (R²={r2:.3f})"
    )
...
plt.savefig(
    os.path.join(
        RESULTS_DIR,
        f"sc{SET_CONSTRAINT}_r2_heatmap.png"
    )
)

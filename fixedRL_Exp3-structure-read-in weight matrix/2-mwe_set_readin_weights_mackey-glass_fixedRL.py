...
def sample_laplace_weights(shape, loc=0.0, scale=0.5):
...
def sample_fourier_weights(shape, max_freq=10):
    ...
    weights = np.sin(2 * np.pi * freqs * t + phases) + \
              np.cos(2 * np.pi * freqs * t + phases)
...
def main():
    ...
    sheet_name_median = 'Exp3_Mackey-Glass_Median'
    sheet_name_iqr = 'Exp3_Mackey-Glass_IQR'
    ...
    for outer in range(args.n_trials):
        ...
        for inner in range(args.n_trials):
            ...
            W_lap = sample_laplace_weights((200, 2))

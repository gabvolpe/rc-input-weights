...
Zeros or close-to zero values are allowed in the read-in matrix.
...
def generate_narma10_data(n_samples=20000):
    ...
    data = np.column_stack([np.arange(n_samples), u, y])
    return data

def narma10_pred(n_batch, n_time_in, n_time_out, n_states=2):
    data = generate_narma10_data(n_samples=n_batch + n_time_in + n_time_out)
...
def sequence_to_scalar(n_batch=200, n_states=2):
    return narma10_pred(n_batch=n_batch, n_time_in=1, n_time_out=1, n_states=n_states)

def sample_laplace(shape, loc=0.0, scale=0.5):
    weights = np.random.laplace(loc=loc, scale=scale, size=shape)
    return weights

def sample_fourier_weights(shape, max_freq=10):
    n_nodes, n_inputs = shape
    t = 0.5  # Use a fixed time point (different from Code 1 where t=0)
    ...
    weights = 0.5 * weights / np.max(np.abs(weights))  # Normalize to [-0.5, 0.5]
...
def main():
    ...
    sheet_name_median = 'Exp3_NARMA10_Median'
    sheet_name_iqr = 'Exp3_NARMA10_IQR'
